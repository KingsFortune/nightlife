...
RegisterKeyMapping('nitros', 'Toggle Nitro', 'keyboard',  Customize.NitroControl)
            ...
            if tonumber(NitroVeh[Plate]) > 0 then
                if GetPedInVehicleSeat(InVehicle, -1) == PlayerPed then
                    while isPressing do
                        ...
                        if GetPedInVehicleSeat(InVehicle, -1) == PlayerPed then
                            local force = Customize.NitroForce
                            NitroVeh[Plate] = NitroVeh[Plate] - Customize.RemoveNitroOnpress
                            SendReactMessage('setNitro', NitroVeh[Plate])
